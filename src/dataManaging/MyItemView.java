...
import httpManaging.HttpManager;
import tools.CommandExecutor;
import tools.exceptions.CancelledCommandException;

public class MyItemView extends JPanel implements ActionListener {
	...
	// TODO file browser, check if file exists and print red text if it doesn't
	// > advising to relocate or delete
	...
	public MyItemView(MyListView<?> parent, Item item) {
		...
		editButton = new JButton(item.editText()); // we can change the button's
													// text with item.editText
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == deleteButton) {
			...
			try {
				// If the user clicks on the edit Button, we modify the item
				// through the corresponding GUI.
				parent.getDataList().editItem(item);

				// Here, the user didn't cancel the item editing => we have to
				// upload the view
			...
			// Just in case of problem, we generate the folders necessary to
			// receive the job.
			...
			if (!httpManager.connected()) {
				// if we are not connected, we open a connection pane and we
				// create a new HttpManager... not so good but whatever
				new ApiConnectionPane().display();
				httpManager = new HttpManager();
			}

			try {
				httpManager.retrieveTransformationFiles(item);
			...
			/*
			 * TODO this piece of codes replaces the get if the jobs are already
			 * stored in local folder transformation/jobs/source/<identifier>
			 * (debug purposes) File sources = new
			 * File("transformation/jobs/source/" + item.getIdentifier());
			 * File[] jobs = sources.listFiles();
			 * 
			 * 
			 * //then we put the new jobs for(int i = 0; i<jobs.length ; i++) {
			 * 
			 * try { Path p = Paths.get(destination.getAbsolutePath(),
			 * jobs[i].getName()); Files.copy(jobs[i].toPath(), p,
			 * StandardCopyOption.COPY_ATTRIBUTES); } catch (IOException e1) {
